...
const SpriteData GrrrolSpawnerSpriteData = { ProfileId::AC_WAKI_GRRROL, 0, 0, 0, 0, 0x100, 0x100, 0, 0, 0, 0, 0 };
Profile GrrrolSpawnerProfile(&daGrrrolSpawner_c::build, SpriteId::AC_WAKI_GRRROL, &GrrrolSpawnerSpriteData, ProfileId::AC_WAKI_GRRROL, ProfileId::AC_WAKI_GRRROL, "AC_WAKI_GRRROL", GrrrolSpawnerArcNameList, 0);
...
bool daGrrrolSpawner_c::spawnGrrrol() {
	...
	dStageActor_c *child = dStageActor_c::create(ProfileId::EN_GRRROL, childSettings, &spawnPos, 0, this->currentLayerID);
	if (!child)
		return false;
...
u32 daGrrrolSpawner_c::countTrackedGrrrols() {
	...
	for (int i = 0; i < 16; i++) {
		...
		if (child && !child->isDeleted && child->profileId == ProfileId::EN_GRRROL) {
